...
class DeliverEventController
{
    ...
    public function __invoke(ServerRequestInterface $request): Response
    {
        ...
        try {
            $this->logger->info('Mailgun Delivered event Received');
            ...
            $mailgunDeliverEvent = new MailgunDeliverEvent();
            ...
            if (null !== $recipient) {
                $mailgunDeliverEvent->setRecipient($recipient);
            }

            $campaign = $this->entityManager->getRepository(Campaign::class)->findOneBy(['campaignNumber' => $req['user-variables']['campaignNumber']]);
            if (null !== $campaign) {
                $mailgunDeliverEvent->setCampaign($campaign);
            }

            if (!empty($req['id'])) {
                $mailgunDeliverEvent->setMailgunEventId($req['id']);
            }

            if (!empty($req['timestamp'])) {
                $mailgunDeliverEvent->setDateDelivered((int) $req['timestamp']);
            }

            $this->entityManager->persist($mailgunDeliverEvent);
            $this->entityManager->flush();

            $this->logger->info(\sprintf('Mailgun Delivered event for recipient %s saved', $mailgunDeliverEvent->getRecipient()->getEmailAddress()));
        } catch (\Exception $ex) {
