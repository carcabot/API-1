...
class MigrateApplicationRequestDigitalDocument extends Command
{
    /**
     * @var DocumentManager
     */
    ...
    {
        $this
            ->setName('app:bridge:migrate-application-request-digital-document')
            ->setDescription('Migrate application request digital document by applicationId')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): ?int
    {
        ...
        if (!empty($id)) {
            ...
            $applicationRequest = $qb->hydrate(Contract::class)
                ...
                ->equals($id)
                ->project()
                ->includeFields(['_applicationId', 'attachments'])
                ->execute();

            if (\iter\count($applicationRequest) > 0) {
                $progressBar = new ProgressBar($output);
                $this->applicationRequestApi->updateApplicationRequestDigitalDocument($applicationRequest);
                foreach ($applicationRequest as $applicationData) {
                    $progressBar->advance();
                    $io->success('Migrated all details of the application request digital document '.$applicationData->getApplicationRequestNumber());
                    $progressBar->finish();
                }
            } else {
            ...
            if (\count($applicationRequestDocument) > 0) {
                $this->logger->info(\sprintf('Migrating all application request digital document ...'));

                $progressBar = new ProgressBar($output);
                $io->text('Migrating application request digital document..... ');
                $this->applicationRequestApi->updateApplicationRequestDigitalDocument($applicationRequestDocument);
                $progressBar->advance();
                $io->success('Migrated all application request digital document.');
                $progressBar->finish();
            } else {
