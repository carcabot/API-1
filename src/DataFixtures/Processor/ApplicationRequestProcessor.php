...
class ApplicationRequestProcessor extends ApplicationRequest implements ProcessorInterface
{
    const LENGTH = 9;
    const PREFIX = 'A';
...
SQL
        ...
        $object->applicationRequestNumber = $this->generate($object);
    ...
    private function generate(ApplicationRequest $applicationRequest)
    {
        $length = self::LENGTH;
        $prefix = self::PREFIX;
        ...
        $applicationRequestNumber = \sprintf('%s%s', $prefix, \str_pad((string) $nextNumber, $length, '0', STR_PAD_LEFT));
