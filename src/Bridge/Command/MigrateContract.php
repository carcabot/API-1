...
class MigrateContract extends Command
{
    ...
    {
        $this
            ->setName('app:bridge:migrate-contract')
            ->setDescription('Migrate contract details by contractId')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): ?int
    {
        ...
        if (!empty($id)) {
            ...
            if (\iter\count($contract) > 0) {
                ...
                $this->contractApi->createContract($contract);
                foreach ($contract as $contractData) {
                    $progressBar->advance();
                    $io->success('Migrated all details of the contract '.$contractData->getContractId());
                    $progressBar->finish();
                }
            } else {
            ...
            if (\count($contractDocument) > 0) {
                $this->logger->info(\sprintf('Migrating all contracts ...'));

                $progressBar = new ProgressBar($output);
                $io->text('Migrating contracts ..... ');
                $this->contractApi->createContract($contractDocument);
                $progressBar->advance();
                $io->success('Migrated all contracts .');
                $progressBar->finish();
            } else {
