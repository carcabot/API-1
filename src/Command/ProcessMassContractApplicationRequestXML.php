...
class ProcessMassContractApplicationRequestXML extends Command
{
    /**
     * @var ContractApplicationRequestCreator
     */
    private $contractApplicationRequestCreator;
    ...
    /**
     * @param ContractApplicationRequestCreator $contractApplicationRequestCreator
     * @param WebServiceClient                  $webServiceClient
     * @param LoggerInterface                   $logger
     * @param string                            $timezone
     * @param string                            $documentConverterHost
     */
    public function __construct(ContractApplicationRequestCreator $contractApplicationRequestCreator, WebServiceClient $webServiceClient, LoggerInterface $logger, string $timezone, string $documentConverterHost)
    {
        parent::__construct();

        $this->contractApplicationRequestCreator = $contractApplicationRequestCreator;
    ...
    {
        $this
            ->setName('app:web-service:process-mass-contract-application-xml')
            ->setDescription('Processes a mass contract application xml file.')
            ...
            ->setHelp(<<<'EOF'
The %command.name% command processes a mass contract application xml file.
EOF
            )
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): ?int
    {
        ...
        if (null !== $fileUrl) {
            ...
            $tempFile = $this->webServiceClient->getMassContractApplicationRequestXMLFile($date);
        }

        $baseUri = HttpUri::createFromString($this->documentConverterHost);
        $modifier = new AppendSegment('application_requests/xml/mass_contract_application');
        ...
        if (null !== $tempFile && \filesize(\stream_get_meta_data($tempFile)['uri']) > 0) {
            ...
            if (200 === $uploadResponse->getStatusCode()) {
                $resultData = $this->contractApplicationRequestCreator->processArrayData($uploadResult);
                if (\count($resultData) > 0) {
                    $this->webServiceClient->uploadReturnFile($resultData, new \DateTime(), UploadFileType::CONTRACT_APPLICATION_RETURN, $upload);
                } else {
