...
class MigrateContractRefundee extends Command
{
    /**
     * @var DocumentManager
     */
    ...
    {
        $this
            ->setName('app:bridge:migrate-contract-refundee')
            ->setDescription('Migrate contract refundee by contractId')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): ?int
    {
        ...
        if (!empty($id)) {
            ...
            $contract = $qb->hydrate(Contract::class)
                ...
                ->equals($id)
                ->project()
                ->includeFields(['_contractId', 'customer_id', 'refund_payee_name', 'refund_payee_nric'])
                ->execute();

            if (\iter\count($contract) > 0) {
                $progressBar = new ProgressBar($output);
                $this->contractApi->updateContractRefundee($contract);
                foreach ($contract as $contractData) {
                    $progressBar->advance();
                    $io->success('Migrated all details of the contract refundee '.$contractData->getContractId());
                    $progressBar->finish();
                }
            } else {
            ...
            if (\count($contractDocument) > 0) {
                $this->logger->info(\sprintf('Migrating all contract refundee...'));

                $progressBar = new ProgressBar($output);
                $io->text('Migrating contract refundee..... ');
                $this->contractApi->updateContractRefundee($contractDocument);
                $progressBar->advance();
                $io->success('Migrated all contract refundee.');
                $progressBar->finish();
            } else {
