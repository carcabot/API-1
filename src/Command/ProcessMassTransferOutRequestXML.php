...
class ProcessMassTransferOutRequestXML extends Command
{
    /**
     * @var ApplicationRequestTransferOutCreator
     */
    private $applicationRequestTransferOutCreator;
    ...
    /**
     * @param ApplicationRequestTransferOutCreator $applicationRequestTransferOutCreator
     * @param WebServiceClient                     $webServiceClient
     * @param LoggerInterface                      $logger
     * @param string                               $timezone
     * @param string                               $documentConverterHost
     */
    public function __construct(ApplicationRequestTransferOutCreator $applicationRequestTransferOutCreator, WebServiceClient $webServiceClient, LoggerInterface $logger, string $timezone, string $documentConverterHost)
    {
        parent::__construct();

        $this->applicationRequestTransferOutCreator = $applicationRequestTransferOutCreator;
    ...
    {
        $this
            ->setName('app:web-service:process-mass-transfer-out-application-xml')
            ->setDescription('Processes a mass transfer out application request xml file.')
            ...
            ->setHelp(<<<'EOF'
The %command.name% command processes a mass mass transfer out application xml file.
EOF
            )
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): ?int
    {
        ...
        if (null !== $fileUrl) {
            ...
            $tempFile = $this->webServiceClient->getMassContractTransferOutApplicationRequestXMLFile($date);
        }

        $baseUri = HttpUri::createFromString($this->documentConverterHost);
        $modifier = new AppendSegment('application_requests/xml/mass_transfer_out');
        ...
        if (null !== $tempFile && \filesize(\stream_get_meta_data($tempFile)['uri']) > 0) {
            ...
            if (200 === $uploadResponse->getStatusCode()) {
                $resultData = $this->applicationRequestTransferOutCreator->processArrayData($uploadResult);
                if (\count($resultData) > 0) {
                    $this->webServiceClient->uploadReturnFile($resultData, new \DateTime(), UploadFileType::TRANSFER_OUT_RETURN, $upload);
                } else {
