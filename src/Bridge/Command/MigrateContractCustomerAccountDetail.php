...
class MigrateContractCustomerAccountDetail extends Command
{
    /**
     * @var DocumentManager
     */
    ...
    {
        $this
            ->setName('app:bridge:migrate-contract-customer-account-detail')
            ->setDescription('Migrate contract customer account details by contractId')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): ?int
    {
        ...
        if (!empty($id)) {
            ...
            if (\iter\count($contract) > 0) {
                ...
                $this->contractApi->updateContractCustomerAccountDetails($contract);
                foreach ($contract as $contractData) {
                    $progressBar->advance();
                    $io->success('Migrated all details of the contract customer account details '.$contractData->getContractId());
                    $progressBar->finish();
                }
            } else {
            ...
            if (\count($contractDocument) > 0) {
                $this->logger->info(\sprintf('Migrating all contract customer account details ...'));

                $progressBar = new ProgressBar($output);
                $io->text('Migrating contract customer account details ..... ');
                $this->contractApi->updateContractCustomerAccountDetails($contractDocument);
                $progressBar->advance();
                $io->success('Migrated all contract customer account details .');
                $progressBar->finish();
            } else {
